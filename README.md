...
This initial section prepares the environment for the application to run, installing the necessary libraries.

*   `!pip install pdfminer.six:` This line installs the pdfminer.six library in the Google Colab environment. This library is essential for extracting text from PDF files.
*   `os:` Allows you to interact with the operating system, such as accessing files and directories.
*   `re:` Allows you to use regular expressions, which are text patterns used to search and manipulate text.
*   `pdfminer.high_level:` Provides functions to extract text from PDF files.
*   `google.colab import drive:` Allows you to connect to Google Drive in the Google Colab environment.
*   `csv:` Allows you to work with CSV files, which are comma-separated data tables.
*   `textwrap:` Allows you to format the output text, breaking long lines to improve readability.
*   `json:` Allows you to work with JSON files, which are text files that store data in a structured format.


## 📂 Section 2: Connecting to Google Drive:
This section explains that the goal is to connect the code to Google Drive to access the PDF files.
`drive.mount('/content/drive'):` This line of code mounts Google Drive in the Google Colab environment.

This allows the script to access Google Drive files as if they were on the Colab file system itself.
...
1. **Reading the PDF:**
   - `pdf_reader = PyPDF2.PdfReader(pdf_path)`:  The `PdfReader` function from the PyPDF2 library is used to open the PDF file specified by the `pdf_path` path. The `pdf_reader` object represents the open PDF.
...
**In summary:** The `extract_text_from_pdf` function takes the path of a PDF file as input, opens the file, iterates through each page and extracts the text from each page, concatenating the text into a single string that is returned as a result.


## 🔍 Section 4: Function to search for words or phrases in the text extracted from the PDF

This section defines the `search_in_text` function, responsible for searching for the provided search terms within the text extracted from the PDF and returning a list with the lines that contain the terms.
...
2. **Separating the Text into Lines:**
...
4. **Storing the Results:**
   - `if term in results:`:  Checks if the search term already exists as a key in the `results` dictionary. If it exists, it means that lines with that term have already been found, so the current line is added to the list of lines corresponding to the term.
   - `else:`:  If the term does not yet exist as a key in the `results` dictionary, it means that this is the first line found with that term, so a new list is created for the term and the current line is added to that list.
...
This section defines the `process_pdfs_in_folder` function, responsible for iterating through all PDF files in a specific folder, extracting the text from each one, performing a term search and storing the results.
...
2. **Iterating through the PDF Files:**
   - `for filename in os.listdir(folder_path):`:  A `for` loop iterates through all files in the folder specified by the `folder_path` path.
   - `if filename.endswith(".pdf"):`:  Checks if the filename ends with ".pdf". If not, the file is not a PDF and is ignored.

3. **Text Extraction and Search:**
   - `pdf_path = os.path.join(folder_path, filename)`:  Creates the full path to the PDF file, combining the folder path and the filename.
...
4. **Storing the Results:**
   - `results[filename] = search_results`:  Stores the search results for the current file (filename as key) in the `results` dictionary.
...
1. **Saving to CSV:**
   ...
   - `for pdf_name, search_results in results.items():`:  A `for` loop iterates over each entry in the `results` dictionary. Each entry represents a PDF (PDF name as key) and the search results for that PDF (dictionary with terms and corresponding lines as value).
   ...
   - `csv_rows.append({'pdf_name': pdf_name, 'line_text': line, 'search_term': search_term})`:  Creates a dictionary with the relevant information (PDF name, line text and search term) and adds that dictionary to the `csv_rows` list.
...
3. **Saving to JSON:**
   ...
   - `json.dump(results, json_file, indent=4)`:  Uses the `json.dump` function to serialize the `results` dictionary into JSON format and write to the file. The `indent=4` argument sets the indentation to improve the readability of the JSON file.

**In summary:** The `save_results` function takes the search results and the output folder path as input. It saves the results in three different formats: CSV, TXT and JSON. The CSV format is created using a Pandas DataFrame, the TXT format is created using file writing operations and the JSON format is created using the `json.dump` function.


## 📁 Section 7: Function to create the output directory if it doesn't exist
...
2. **Creating the Folder:**
   - `os.makedirs(output_folder)`:  The `makedirs` function of the `os` module is used to create the folder specified by the `output_folder` path, including any parent folders if necessary.
...
2. **Defining the Folder Path and Search Terms:**
   - `folder_path = '/content/drive/MyDrive/PDFs_scraper'`:  Defines the path to the folder containing the PDF files.
...
3. **Processing the PDFs and Obtaining the Results:**
...
4. **Creating the Output Directory:**
   - `output_folder = '/content'`:  Defines the path to the folder where the results will be saved.
...
6. **Displaying the Results in Colab:**
   - `print("Resultados salvos com sucesso em CSV, TXT e JSON.")`:  Prints a message to the Colab screen informing that the results have been saved.
   - `for filename, search_results in results.items():`:  A `for` loop iterates over each PDF and the search results.
   - `print(f"\n{search_term}:")`:  Prints the search term to the Colab screen.
   - `for i, line in enumerate(lines):`:  A `for` loop iterates over each line containing the search term.
   - `print("-" * 140)`:  Prints a separator line between the different responses.
   - `wrapped_text = textwrap.fill(line, width=140)`:  Applies "text wrap" to the line to improve readability.
   - `print(wrapped_text)`:  Prints the line with "text wrap" applied to the Colab screen.

**In summary:** Section 8 is responsible for running the script, connecting to Google Drive, processing the PDFs, saving the results in different formats and displaying the results on the Colab screen.
